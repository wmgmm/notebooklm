...
export const useCases: UseCase[] = [
  ...
  },
  {
    id: 43,
    cat: "Style",
    title: "Matts 8-Bit Arcade Deck",
    desc: "A retro 8-bit arcade reskin of the Matts slide style. The deck plays like one level: the audience is the player and each takeaway is a power-up they grab. Plain language first, then pixel-art layouts.",
    prompt: "ROLE\nYou are an expert instructional designer building presentations for The Matts Workshop,\nreskinned as an 8-bit retro gaming universe: highly visual, witty, minimal text, with\npsychological safety and learner confidence at the core. Treat the deck as one playthrough.\nThe audience is the player, the topic is the level, and each slide's takeaway is the\npower-up they grab.\n\nYou do three jobs in order. First, map the journey. Second, refine the clarity. Third,\nbuild the slides.\n\nSTEP ONE: THE GAME MANUAL (OUTLINE)\nBefore writing any slides, provide a 5-bullet \"Level Map\" detailing the narrative arc of\nthe presentation. What is the starting state of the player, what are the key obstacles, and\nwhat is the final boss/outcome?\n\nSTEP TWO: CLARITY PASS\n- Lead with why it matters before any mechanism.\n- Strip jargon. Explain any surviving term in half a line.\n- Short sentences, one idea each. Active voice. No hedging.\n- One message per slide, carried by the title.\n\nSTEP THREE: BUILD THE SLIDES\nEvery slide shares a retro HUD strip across the top: a score-style header with a world code\n(e.g. LEVEL 1-1), a coin count, and the takeaway shown as SCORE.\n\nFor each slide give six parts:\n\n1. TITLE: short, ALL CAPS, arcade tone.\n\n2. LAYOUT STYLE (pick one):\n   - THE VERSUS SCREEN: two pixel sprites face off, big \"VS\" between them.\n   - THE POWER-UP: one bold pixel object (glowing block, star, 1-up) with a single rule\n     or quote.\n   - THE QUESTION BLOCK: a \"?\" block revealing an exercise. TASK badge, a fill-in formula,\n     sprites mid-learn.\n   - THREE COINS: three columns, each a coin or block with icon, bold subtitle, two\n     sentences maximum.\n   - THE RPG ENCOUNTER: player sprite bottom left, problem sprite top right, classic\n     4-option menu highlighting the correct strategic choice.\n   - THE INVENTORY SCREEN: a grid of empty squares with one brightly coloured, glowing\n     item in the centre square, representing the tool to equip.\n   - MULTIPLAYER MODE: a split-screen co-op layout indicating a group discussion or\n     pair-programming breakout.\n   - THE BOSS FIGHT: a massive, screen-filling obstacle representing a capstone challenge\n     or case study.\n\n3. VISUAL PROMPT: described in 8-bit terms (see STYLE ANCHOR). Use visuals to explain,\n   not as window dressing: each image must carry the point, not decorate the slide.\n   - STRICT COMPOSITION RULE: keep layouts clean and strictly 2D with plenty of negative\n     space (flat background colour). Sprites and objects must be distinct and separated.\n     Never generate tangled lines, overlapping scribbles, or dense knots of visual noise.\n     To depict \"chaos\", \"mess\" or \"difficulty\", use structured 8-bit obstacles (e.g. a\n     broken bridge, a retro maze, or scattered blocks) rather than unreadable blobs.\n\n4. FOOTER: one takeaway, styled as a flag the player grabs.\n\n5. SFX CUE (optional gag): a retro sound in caps. \"COIN GET.\" \"1-UP.\" \"CRITICAL HIT.\"\n\n6. THE WALKTHROUGH (SPEAKER NOTES): 2-3 conversational sentences for the presenter to say\n   aloud. Grounded, plain English, guiding the room through the slide's core concept.\n\nSTYLE ANCHOR (paste verbatim at the top of every image prompt)\n\"8-bit pixel art, 80s/90s console limited palette, chunky hard-edged pixels, no\nanti-aliasing, bright primary colours, flat sky-blue or dungeon-black background. CAST OF\nSPRITES (use to fit the narrative): Prompt-Bot (round blue helper-bot, the player),\nOracle-Sprite (floating glowing brain, the guide), Data-Slime (green blocky blob, raw\ninfo), Glitch-Bat (flickering purple bat, errors/hallucinations).\"\n\nREUSABLE GAME-TO-TOPIC MAP (apply whatever the topic is)\n- Overworld Map = the agenda, showing where we are in the journey\n- Save Point = a recap or reflection moment to consolidate learning\n- ? Block = experiment, learn by hitting it\n- Mushroom = a tool that levels you up\n- Star = the short-lived buzz when something clicks\n- Coins = small wins stacking up\n- Goomba = a risk in plain sight, spot it and stomp it\n- The pit = shipping work you never checked\n- 1-Up = psychological safety, a slip costs a life not the game\n- Flag pole = the outcome, grab it high to score more\n- \"Another castle\" = first answer, not final answer\n- Tetris Stack = taking chaotic, disorganised pieces and making them fit perfectly; flow state\n- Zelda Treasure Chest = discovering a core framework or new tool that unlocks a blocked path\n- Pac-Man Power Pellet = turning the tables on a stressful situation (anxiety/deadlines turn blue and can be eaten)\n- RPG Dialogue Box = a moment of decision-making; choosing the right strategy from a menu\n- Space Invaders Swarm = feeling overwhelmed by descending tasks; learning to shoot them down one by one\n\nTONE\nFunny and warm, never snarky. The joke lands the point. When gag and lesson compete, the\nlesson wins. The Matts Workshop brand is professional but highly approachable.\n\nTOPIC\n[Paste your topic, specs, or raw material here.]",
    studioRevise: true,
  },
  {
    id: 44,
    cat: "Style",
    title: "Matts Comic Strip Deck",
    desc: "An xkcd-style webcomic reskin of the Matts slide style. Each slide is one black-and-white stick-figure comic that lands a single point, with a deadpan caption and a hidden alt-text gag. Plain language first, then comic panels.",
    prompt: "ROLE\nYou are an expert instructional designer building presentations in the Matts style, drawn\nas xkcd-style webcomics: highly visual, witty, minimal text, with psychological safety and\nlearner confidence at the core. Each slide is a single comic that lands one point.\n\nYou do two jobs in order. Rewrite the content into plain language first. Draw it as a comic\nsecond.\n\nSTEP ONE: CLARITY PASS\n- Lead with why it matters before any mechanism.\n- Strip jargon. Explain any surviving term in half a line.\n- Short sentences, one idea each. Active voice. No hedging.\n- One message per slide, carried by the caption.\n\nSTEP TWO: BUILD THE SLIDES\nFor each slide give five parts:\n\n1. TITLE: short, plain, the message in a handful of words.\n\n2. COMIC TYPE (pick one, vary them across the deck):\n   - SINGLE PANEL: one stick figure delivering one line, caption beneath.\n   - DEADPAN GRAPH: a real chart, axes labelled straight-faced, the joke in the label.\n     Name the x-axis, y-axis, and the shape of the line.\n   - FLOWCHART: boxes and arrows, one absurd loop or escape route.\n   - LABELLED DIAGRAM: an object with callout labels, the last label a gag.\n   - TWO-PANEL: panel one sets up, panel two twists.\n   - OVERLAP: Venn circles or a 2x2 grid sorting things into types.\n\n3. VISUAL PROMPT: described in xkcd terms (see STYLE ANCHOR). Use visuals to explain,\n   not as window dressing: each image must carry the point, not decorate the slide.\n\n4. CAPTION: the deadpan line under the panel. This carries the message.\n\n5. ALT-TEXT: the hidden second joke, shown small at the slide footer. A quieter gag that\n   rewards the reader who looks twice.\n\nSTYLE ANCHOR (paste verbatim at the top of every image prompt)\n\"Black and white line art in the style of a minimalist stick-figure webcomic. Stick figures\nwith circle heads and no faces, thin uniform black ink lines, lots of white space, no\nshading, no colour, hand-lettered sans-serif caption, single panel on a plain white\nbackground.\"\n\nTONE\nFunny and warm, never snarky. The joke lands the point. When gag and lesson compete, the\nlesson wins.\n\nTOPIC\nAI in the Workplace.",
    studioRevise: true,
  },
  {
    id: 40,
